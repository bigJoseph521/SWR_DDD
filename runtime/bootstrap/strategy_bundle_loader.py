...
def raw_dict_to_launch_payload(
    ...
    *,
    ...
    for k in ("runtime_id", "tenant_id", "strategy_version_id", "mode", "entrypoint"):
        put_if_key(k)
    ...
    work_root = (
        str((base_dir / work_root_json).resolve())
        if work_root_json.strip()
        else str((base_dir / ".swr-runtime").resolve())
    )
...
def effective_bar_timeframe(data: dict[str, Any]) -> str:
    """
    Bar cadence for backtest historical/replay and for PAPER/LIVE bar tick labeling / stream choice.
...
def extract_runtime_tuning(
    ...
    *,
    ...
    historical_data_grpc_target = str(
        _env_or_scalar(
            "SWR_HISTORICAL_DATA_GRPC_TARGET", "historical_data_grpc_target", ""
        )
    ).strip()
    ...
    historical_data_grpc_timeout_seconds = _float_env_or_bundle(
        "SWR_HISTORICAL_DATA_GRPC_TIMEOUT_SECONDS",
        "historical_data_grpc_timeout_seconds",
        30.0,
    )
    ...
    try:
        backtest_bar_replay_interval_ms = int(
            os.environ.get("SWR_BACKTEST_BAR_REPLAY_INTERVAL_MS")
            or (
                ""
                if connectivity_env_only
                else _scalar(data, "backtest_bar_replay_interval_ms", 100)
            )
            or 100
        )
    except (TypeError, ValueError):
        backtest_bar_replay_interval_ms = 100
    backtest_bar_replay_interval_ms = max(0, backtest_bar_replay_interval_ms)
    ...
    return {
        ...
        "worker_control_grpc_bind": str(
            _scalar(data, "worker_control_grpc_bind", "127.0.0.1:50051")
        ),
        "worker_control_grpc_fallback_ports": str(
            _scalar(data, "worker_control_grpc_fallback_ports", "")
        ),
        "worker_control_grpc_no_fallback": _truthy(
            data.get("worker_control_grpc_no_fallback", False)
        ),
        ...
        "replay_worker_grpc_target": str(
            _scalar(data, "replay_worker_grpc_target", "")
        ),
        "replay_grpc_timeout_seconds": float(
            _scalar(data, "replay_grpc_timeout_seconds", 3)
        ),
        ...
        "historical_data_grpc_target": historical_data_grpc_target,
        "historical_data_grpc_timeout_seconds": historical_data_grpc_timeout_seconds,
        ...
        "backtest_bar_replay_interval_ms": backtest_bar_replay_interval_ms,
