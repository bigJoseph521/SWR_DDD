...
def build_sdk_order_intent_submitter(
    *,
    ...
    if not isinstance(gateway, RiskOrderIntentGateway):
        return None
    risk_inner = getattr(gateway, "_risk_client", None)
    can_submit = callable(getattr(risk_inner, "submit_order_intent", None))
    ...
    submission_port = RiskGatewaySubmissionAdapter(
        gateway,
        submission_context=submission_context,
    )
    submit_use_case = SubmitOrderIntent(submission_port=submission_port)
    ...
    def _submit(sdk_intent: Any) -> dict[str, Any]:
        ...
        try:
            ...
            if disable_order_intent_grpc or not can_submit:
            ...
            _LOG.error(
                "risk_order_intent_submit_prepare_failed",
        ...
        if on_order_intent_result is not None:
            try:
                on_order_intent_result("risk", wire_payload, result)
            except Exception:
                _LOG.exception("on_order_intent_result_callback_failed")
        return result

    if (not disable_order_intent_grpc and can_submit) or on_order_intent_result is not None:
        return _submit
    return None
