...
class RiskOrderIntentGateway:
    def __init__(
        self,
        policy: ModePolicy,
        risk_order_intent_client: object,
        *,
        ...
        self._policy = policy
        self._risk_client = risk_order_intent_client
    ...
    def submit_order_intent_wire(self, payload: Mapping[str, Any]) -> Any:
        ...
        sender = getattr(self._risk_client, "submit_order_intent", None)
        if not callable(sender):
            raise TypeError(
                "Risk order intent client must expose submit_order_intent(payload)."
            )
    ...
    def submit_order_intent_payload(self, payload: Mapping[str, Any]) -> Any:
        """Deprecated alias for :meth:`submit_order_intent_wire`."""
        return self.submit_order_intent_wire(payload)
    ...
    def submit_replace_order_intent_payload(self, payload: Mapping[str, Any]) -> Any:
        ...
        sender = getattr(self._risk_client, "submit_replace_order_intent", None)
        if not callable(sender):
            raise TypeError(
                "Risk order intent client must expose submit_replace_order_intent(payload)."
            )
        out = sender(normalized_payload)
        self._notify_order_intent_result(normalized_payload, out)
        return out
    ...
    def submit_cancel_order_intent_payload(self, payload: Mapping[str, Any]) -> Any:
        ...
        sender = getattr(self._risk_client, "submit_cancel_order_intent", None)
        if not callable(sender):
            raise TypeError(
                "Risk order intent client must expose submit_cancel_order_intent(payload)."
            )
        out = sender(normalized_payload)
        self._notify_order_intent_result(normalized_payload, out)
        return out
