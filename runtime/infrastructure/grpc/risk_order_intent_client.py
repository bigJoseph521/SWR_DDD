...
def normalize_risk_order_intent_payload(payload: Mapping[str, Any]) -> dict[str, Any]:
    """
    Merge legacy domain/SDK dicts into the canonical worker→Risk Service shape (``risk_worker.proto``).
    """
    raw = dict(payload)
    # Backtest / replay-only; must not appear on Risk Service wire or journal payloads.
...
def order_intent_wire_dict_for_console(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    """
    p = normalize_risk_order_intent_payload(dict(payload))
    for key in ("tenant_id", "worker_identity", "deployment_id"):
...
def _optional_extension(payload: Mapping[str, Any]) -> Any:
    ...
    if not any(
        ...
        for k in ("runtime_id", "strategy_version_id", "tenant_id")
    ):
        return None
...
def _normalize_grpc_error(exc: grpc.RpcError) -> DependencyClientError:
    ...
    if status_code == grpc.StatusCode.NOT_FOUND:
        return DependencyClientError(
            ...
            message="Risk Service dependency entity not found.",
            retryable=False,
            details=details,
        )
    if status_code == grpc.StatusCode.INVALID_ARGUMENT:
        return DependencyClientError(
            ...
            message="Risk Service dependency rejected request validation.",
            retryable=False,
            details=details,
        )
    if status_code in {
        ...
        return DependencyClientError(
            ...
            message="Risk Service dependency denied request by policy/access.",
            retryable=False,
            details=details,
        )
    ...
    return DependencyClientError(
        ...
        message="Risk Service dependency call failed unexpectedly.",
        retryable=False,
        details=details,
    )


class RiskOrderIntentGrpcClient:
    def __init__(
        self,
        stub: Any,
        *,
    ...
    def submit_order_intent(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        p = normalize_risk_order_intent_payload(payload)
        wall = p.get("requested_at")
...
def build_risk_order_intent_grpc_client(
    *, target: str, timeout_seconds: float = 3.0
) -> RiskOrderIntentGrpcClient | None:
    """When ``target`` is non-empty (``host:port``), returns a :class:`RiskOrderIntentGrpcClient`."""
    t = (target or "").strip()
    if not t:
        return None
    channel = grpc.insecure_channel(t)
    stub = risk_worker_pb2_grpc.OrderIntentServiceStub(channel)
    return RiskOrderIntentGrpcClient(stub, timeout_seconds=timeout_seconds)
