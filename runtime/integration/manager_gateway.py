...
from runtime.events.dedupe import DedupeDecision, LifecycleSignalDedupe
from runtime.events.event_envelope import LifecycleEventEnvelope
from runtime.events.event_factory import LifecycleEventFactory
...
class ManagerGateway:
    ...
    def _emit(self, *, signal_type: str, payload: Mapping[str, Any]) -> Any:
        ...
        lifecycle_event = self._event_factory.create(
            ...
            causation_id=getattr(self._runtime_identity, "causation_id", None),
            triggering_event_id=(
                str(payload["triggering_event_id"])
                if "triggering_event_id" in payload
                and payload["triggering_event_id"] is not None
                else None
            ),
        )
