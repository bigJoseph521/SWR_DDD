...
SWR_HISTORICAL_DATA_GRPC_TARGET = "127.0.0.1:50051"
SWR_HISTORICAL_DATA_GRPC_TIMEOUT_SECONDS = 30
...
SWR_BACKTEST_BAR_REPLAY_INTERVAL_MS = 100
...
_DOTENV_BLOCKLIST: frozenset[str] = frozenset(
    {
        ...
        "SWR_HISTORICAL_DATA_GRPC_TARGET",
        "SWR_HISTORICAL_DATA_GRPC_TIMEOUT_SECONDS",
        "SWR_OMS_GRPC_TARGET",
        ...
        "SWR_BACKTEST_BAR_REPLAY_INTERVAL_MS",
    ...
    merged.update(
        {
            ...
            "historical_data_grpc_target": SWR_HISTORICAL_DATA_GRPC_TARGET,
            "historical_data_grpc_timeout_seconds": float(
                SWR_HISTORICAL_DATA_GRPC_TIMEOUT_SECONDS
            ),
            ...
            "backtest_bar_replay_interval_ms": int(SWR_BACKTEST_BAR_REPLAY_INTERVAL_MS),
        }
    )
    return merged
...
class Settings:
    ...
    deployment_id: str
    worker_control_grpc_bind: str
    worker_control_grpc_fallback_ports: str
    worker_control_grpc_no_fallback: bool
    ...
    #: ``host:port`` for risk-service hot path gRPC (``risk_worker.proto`` / ``OrderIntentService``).
    ...
    replay_worker_grpc_target: str
    replay_grpc_timeout_seconds: float
    ...
    historical_data_grpc_timeout_seconds: float
    historical_bars_page_size: int
    #: BACKTEST: wall-clock pause after each historical bar is ingested (0 = no delay).
    backtest_bar_replay_interval_ms: int = 100
...
def _settings_from_prepared(
    *,
    ...
    if print_launch_banner:
        ...
        if risk_t:
            print(f"risk service hot-path target: {risk_t}")
            print(f"risk service hot-path TCP status: {_grpc_target_status(risk_t)}")
        else:
            oms_t = str(tuning.get("oms_grpc_target", "")).strip()
            if oms_t:
                print(f"OMS gRPC target: {oms_t}")
                print(f"OMS gRPC status: {_grpc_target_status(oms_t)}")
    ...
    return Settings(
        ...
        worker_control_grpc_bind=str(tuning["worker_control_grpc_bind"]),
        worker_control_grpc_fallback_ports=str(
            tuning["worker_control_grpc_fallback_ports"]
        ),
        worker_control_grpc_no_fallback=bool(tuning["worker_control_grpc_no_fallback"]),
        ...
        replay_worker_grpc_target=str(tuning["replay_worker_grpc_target"]),
        replay_grpc_timeout_seconds=float(tuning["replay_grpc_timeout_seconds"]),
        ...
        oms_correlation_id=str(tuning["oms_correlation_id"]),
        replay_session_id=str(tuning["replay_session_id"]),
        disable_order_intent_grpc=bool(tuning["disable_order_intent_grpc"]),
        historical_data_grpc_target=str(tuning["historical_data_grpc_target"]),
        historical_data_grpc_timeout_seconds=float(
            tuning["historical_data_grpc_timeout_seconds"]
        ),
        historical_bars_page_size=max(
            1, int(_scalar(data, "historical_bars_page_size", 1000) or 1000)
        ),
        backtest_bar_replay_interval_ms=int(tuning["backtest_bar_replay_interval_ms"]),
